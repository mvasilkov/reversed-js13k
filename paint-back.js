...
function prepareStars(opacity, size) {
  var stars = [];
  for (var i=0; i < 100; i++) {
    stars.push({x:rnd(0, cwidth), y:rnd(0, cheight), o: opacity, off: 0, size: size});
  }
  return stars;
}
...
function paintBackdrop(direction) {
  drawStars(stars1, 2 * direction);
  drawStars(stars2, 8 * direction);
  drawStars(stars3, 15 * direction);
}
